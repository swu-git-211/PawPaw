...
const EditProfile = ({ user, onSave }) => {
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(`${process.env.REACT_APP_API_URL}/pawpaw/profile`, data, {
        headers: {
  ...
  return (
    <Box>
      ...
      <form>
        {profileImage && (
          <img src={`${process.env.REACT_APP_API_URL}/pawpaw/${profileImage}`} alt="Profile" style={{ width: '100px', height: '100px', objectFit: 'cover' }} />
        )}
        <TextField
          label="Bio"
          name="bio"
