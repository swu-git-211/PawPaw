...
const Comment = ({ postId, initialComments }) => {
  ...
  const fetchComments = async () => {
    try {
      const response = await axios.get(`http://3.214.235.164:5000/pawpaw/posts/${postId}/comments`);
    ...
    if (replyContent && replyContent.trim() !== '') {
      try {
        ...
        const response = await axios.post(
          `http://3.214.235.164:5000/pawpaw/posts/${postId}/comment`,
          {
  ...
  const handleLikeComment = async (commentId) => {
    ...
    try {
      const response = await axios.post(
        `http://3.214.235.164:5000/pawpaw/comments/${commentId}/like`,
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      );
      if (response.data.success) {
  ...
  const handleDeleteComment = async (commentId) => {
    try {
      ...
      const response = await axios.delete(`http://3.214.235.164:5000/pawpaw/comment/${commentId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
