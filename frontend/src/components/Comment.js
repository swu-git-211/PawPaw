...
const Comment = ({ postId, initialComments }) => {
  ...
  const fetchComments = async () => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_API_URL}/pawpaw/posts/${postId}/comments`);
    ...
    if (replyContent && replyContent.trim() !== '') {
      try {
        ...
        const response = await axios.post(
          `${process.env.REACT_APP_API_URL}/pawpaw/posts/${postId}/comment`,
          {
  ...
  const handleLikeComment = async (commentId) => {
    ...
    try {
      const response = await axios.post(
        `${process.env.REACT_APP_API_URL}/pawpaw/comments/${commentId}/like`,
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      );
      if (response.data.success) {
  ...
  const handleDeleteComment = async (commentId) => {
    try {
      ...
      const response = await axios.delete(`${process.env.REACT_APP_API_URL}/pawpaw/comment/${commentId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
